...
#if defined(NDEBUG)
# error "Communistcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8ad32fe786b41f8f0330853180f9ad0badbef94c75c6f598e68ae337241e2156");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Communistcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Communistcoin Signed Message:\n";
...
{
    // Communistcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 1 * 60; // Communistcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Communistcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Communistcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8ad32fe786b41f8f0330853180f9ad0badbef94c75c6f598e68ae337241e2156");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Revolution is the only solution";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x029a99cd59a5b90fbc4edba06401bf8ba844a3b3d7da6358928e1e9f61580941"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Communistcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CommunistcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CommunistcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CommunistcoinMiner(CWallet *pwallet)
{
    printf("CommunistcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("communistcoin-miner");
    ...
    try { loop {
        ...
        printf("Running CommunistcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CommunistcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CommunistcoinMiner, pwallet));
}
